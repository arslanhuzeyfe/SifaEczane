-- =====================================================================
--  ECZANE OTOMASYON SİSTEMİ - Veritabanı Tam Kurulum Script'i
--  BSM218/BSM303 Veritabanı Yönetim Sistemleri - Final Ek Ödev
--  MySQL 8.0+
-- =====================================================================
--  Bu dosyayı tek seferde MySQL Workbench veya phpMyAdmin'de
--  çalıştırarak tüm veritabanını kurabilirsin.
-- =====================================================================

-- Eski kurulum varsa temizle (isteğe bağlı)
...
-- =====================================================================
--  2. STORED PROCEDURES (CRUD ve raporlar)
-- =====================================================================
...
BEGIN
    SELECT
        ilac_id AS ID, ilac_ad AS Ad, ilac_kategori AS Kategori,
        ilac_uretici AS Uretici, ilac_fiyat AS Fiyat,
        ilac_stok AS Stok, ilac_son_kullanma AS SonKullanma,
...
BEGIN
    SELECT * FROM ec_ilaclar
    WHERE ilac_stok < p_esik
    ORDER BY ilac_stok ASC;
END $$
DELIMITER ;
...
BEGIN
    SELECT
        s.satis_id                                  AS SatisID,
        CONCAT(m.musteri_ad,' ',m.musteri_soyad)    AS Musteri,
        m.musteri_sigorta                           AS Sigorta,
        i.ilac_ad                                   AS Ilac,
        i.ilac_kategori                             AS Kategori,
        s.satis_adet                                AS Adet,
        i.ilac_fiyat                                AS BirimFiyat,
        s.satis_fiyat                               AS ToplamFiyat,
        s.satis_odeme_tur                           AS OdemeTuru,
        s.satis_tarih                               AS SatisTarihi
    FROM ec_satislar s
...
BEGIN
    INSERT INTO ec_odemeler
        (odeme_id, musteri_id, odeme_tarih,
         odeme_tutar, odeme_tur, odeme_aciklama)
...
BEGIN
    ...
    SET musteri_id = p_musteri_id, odeme_tarih = p_tarih,
        odeme_tutar = p_tutar, odeme_tur = p_tur,
...
BEGIN
    ...
    INTO   v_stok, v_skt, v_ad
    FROM ec_ilaclar WHERE ilac_id = NEW.ilac_id;

    IF NEW.satis_adet > v_stok THEN
        SET v_msg = CONCAT('HATA: ', v_ad, ' icin stok yetersiz! ',
                           'Istenen: ', NEW.satis_adet,
                           ' / Mevcut: ', v_stok);
...
BEGIN
    UPDATE ec_ilaclar
    SET    ilac_stok = ilac_stok - NEW.satis_adet
...
-- =====================================================================
--  5. ÖRNEK VERİ (Test için)
-- =====================================================================

CALL sp_MusteriEkle('M001', 'Ahmet',  'Yilmaz',  '12345678901',
                    '05551112233', 'ahmet@mail.com', 'Bartin Merkez', 'SGK');

CALL sp_MusteriEkle('M002', 'Ayse',   'Demir',   '12345678902',
                    '05552223344', 'ayse@mail.com',  'Bartin Merkez', 'Ozel');

CALL sp_MusteriEkle('M003', 'Mehmet', 'Kara',    '12345678903',
                    '05553334455', 'mehmet@mail.com','Bartin Merkez', 'Yok');

CALL sp_IlacEkle('I001', 'Parol 500mg',     'Agri Kesici', 'Atabay',
                 12.50, 100, '2027-12-31', 0);

CALL sp_IlacEkle('I002', 'Augmentin BID',   'Antibiyotik', 'GSK',
                 85.00, 30,  '2026-09-15', 1);

CALL sp_IlacEkle('I003', 'C Vitamini 1000', 'Vitamin',     'Roche',
                 65.00, 50,  '2027-06-20', 0);

CALL sp_IlacEkle('I004', 'Aspirin 100mg',   'Agri Kesici', 'Bayer',
                 18.75, 200, '2028-03-10', 0);
...
-- =====================================================================
--  6. TEST SORGULARI (raporlama için)
-- =====================================================================

-- Satış detayları
CALL sp_SatisDetay();

-- M001 müşterisinin bakiyesi
SELECT fn_MusteriBakiye('M001') AS M001_Bakiye;

-- Nisan 2026 cirosu
SELECT fn_AylikCiro(2026, 4) AS NisanCirosu;

-- Stoğu 50'den az olan ilaçlar
CALL sp_StoguAzalanIlaclar(50);

-- Tüm ilaçların son kullanma tarihine kalan gün
SELECT ilac_ad, fn_SKTKalanGun(ilac_id) AS KalanGun
FROM ec_ilaclar
ORDER BY KalanGun ASC;

-- Trigger testi: stoktan fazla satış denemesi (HATA vermeli)
-- CALL sp_SatisEkle('S999','M001','I002', 9999, NOW(), 100, 'Nakit');
